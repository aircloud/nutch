...
public class FetchItemQueues {
  ...
  public synchronized FetchItem getFetchItem() {
    ...
    while (it.hasNext()) {
      ...
      if (fiq.getQueueSize() == 0 && fiq.getInProgressSize() == 0) {
        if (maxExceptionsPerQueue > -1 && fiq.exceptionCounter.get() > 0) {
  ...
  public synchronized int checkExceptionThreshold(String queueid,
    ...
    if (delay > 0) {
      ...
      // delay the next fetch by a time span by a time span growing at log scale
